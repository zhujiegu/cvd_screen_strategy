...
mixoutsamp=function(model,newdata){
  ...
  for(i in 1:length(split.obs.ids)){
    ...
    # variance of BLUP
    var.reffects[[i]] = G - (G%*%t.z.mat.oids)%*%
      solve((z.mat.oids%*%G%*%t.z.mat.oids+resid.var.mat*corr.mat))%*%(z.mat.oids%*%t(G))
  }
  ...
  names(var.reffects) <- names(split.obs.ids)
  
  list(preddata=preddata,random=random,var_ref=var.reffects)
}
