...
for(l in 0:GLOB_horizon)
{
  ...
  cens_pred_5CVD = survfit(Surv(dati_baseline[derivation == "validation" & lm_subcohort_index == 1, time_cvd],
                                1 - dati_baseline[derivation == "validation" & lm_subcohort_index == 1, 
                                                  status_cvd]) ~ 1)
  ...
  if(l == 0){
    ...
    save(pred_error_10CVD, file = paste0(out_path,"pred_error_10CVD_", mod_type, j,"_", gender, ".RData"))
    save(data_cindex_10CVD, file = paste0(out_path,"data_cindex_10CVD_", mod_type, j,"_", gender, ".RData"))
    ...
    save(pred_error_10death, file = paste0(out_path,"pred_error_10death_", mod_type, j, "_", gender, ".RData"))
    save(data_cindex_10death, file = paste0(out_path,"data_cindex_10death_", mod_type, j,"_", gender, ".RData"))
    ...
    save(pred_error_10composite, file = paste0(out_path,"pred_error_10composite_", mod_type, j,"_", gender, ".RData"))
    save(data_cindex_10composite, file = paste0(out_path,"data_cindex_10composite_", mod_type, j,"_", gender, ".RData"))
  ...
  risk_prediction[ patid %in% dati_baseline[derivation == "derivation", patid], paste0("5.y.risk.",l) := five_year_risk  ]
  risk_prediction[ patid %in% dati_baseline[derivation == "derivation", patid], paste0("5.y.risk.status.",l) := ifelse(five_year_risk <= 0.95, 1, 0) ]
  hitting_time = apply(estimated_surv, 2, function(x) which(x  <= 0.95)[1] )
  risk_prediction[ patid %in% dati_baseline[derivation == "derivation", patid], paste0("5.y.pred.time.",l):=  base_haz_5CVD[ hitting_time, 2 ] ]
...
#prediction errors 
save(data_cindex_5CVD, file = paste0(out_path, "data_cindex_5CVD_", mod_type, j,"_", gender, ".RData"))
save(pred_err_total_5CVD, file = paste0(out_path, "pred_error_5CVD_", mod_type, j, "_", gender, ".RData"))

save(risk_prediction, file = paste0(out_path, "risk_pred_FE4_", mod_type, j,"_", gender, ".RData"))
save(pt_analysed, file = paste0(out_path,"pt_analysed_FE4_", mod_type, j,"_", gender, ".RData"))
