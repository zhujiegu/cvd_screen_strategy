...
# adjusting from 5% to 15%
risk_pred_now <- risk_pred_now %>% dplyr::select(!contains('status')) %>% dplyr::select(!contains('time'))
for(l in 0:10){
  risk_pred_now <- risk_pred_now %>% mutate('5.y.risk.status.{l}' := 
                                              ifelse(.data[[paste0('5.y.risk.',l)]] <= 0.85, 1, 0))
}
...
hist(baseline_risk)
baseline_class = ifelse( baseline_risk <= 0.075/2, "Low risk",
                         ifelse( baseline_risk <= 0.15/2, "Med-low risk",
                                 ifelse( baseline_risk <= 0.225/2, "Med-high risk",
                                         ifelse( baseline_risk <= 0.30/2, "High risk", "Very high risk"))))
...
pred_risk = apply( risk_pred_now[ , ..seq_index_risk ], 1, function(x) x[ which( x < 0.85 )[1] ] )
...
pred_risk_before = apply(risk_pred_now[!is.na( pred_time ) & pred_time != 1 , 
                                       ..seq_index_risk ], 1, function(x) x[ which( x < 0.85 )[1] - 1 ] )
...
save(comparing_schemes, file = paste0(out_path,"comparing_schemes_RIRS_apply_", i, "_", gender, "_15%.RData") )
